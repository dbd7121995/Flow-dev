...
function NewCase ($params)
{
    ...
    if (is_array($variables)) {
        foreach ($variables as $key => $val) {
            //Important: The value of "$val->name" could be a vulnerable string, 
            //because it passes through the 'eval' function, filter the incorrect 
            //values according to the case, the string: "'1']['2'" is valid because 
            //Is used for the values of a gridForm of a webentry in classic processes.
            if (is_string($val->name) === true) {
                //variable name must be valid
                if (!preg_match("/^[0-9a-zA-Z\_\[\]]+$/", $val->name)) {
                    $context = \Bootstrap::getDefaultContextLog();
                    $context['action'] = 'soap2';
                    $context['exception'] = "Invalid param: " . G::json_encode($val);
                    \Bootstrap::registerMonolog('soap2', 400, 'NewCase', $context, $context['workspace'], 'processmaker.log');
                    continue;
                }
            }
            if (!is_object($val->value)) {
                //Important: Please, consider that this expression is expecting 
                //the value of "$val->name" to be a valid expression, 
                //for example the string: "'1']['2'", the same one when 
                //completed forms a correct index for The value of '$field'.
