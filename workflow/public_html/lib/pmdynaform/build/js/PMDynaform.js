...
(function () {
    ...
    Selector.prototype.init = function (options) {
        ...
        this.subforms = options.subforms;
    ...
    /**
     * Returns the subform with the supplied id, otherwise returns null.
     * @param id
     * @returns {PMDynaform.view.SubForm|null}
     */
    Selector.prototype.findSubformById = function (id) {
        return this.subforms.find(function (i) {
            return i.model.get("id") === id;
        }) || null;
    };
    Selector.prototype.applyGlobalSelectors = function () {
        ...
        window.getSubformById = function (id) {
            return that.findSubformById(id);
        };
        return this;
    };
        ...
        _setDataToEditMode: function (cell, data) {
            ...
            if (cell && data !== undefined && data !== null) {
                ...
                if (type === "suggest" && typeof cell.setData === "function") {
                    ...
                    cell.setValue(fixedData['value'] || false);
                } else {
                    if (typeof cell.setValue === "function") {
                        cell.model.set({"data": fixedData}, {silent: true});
                        cell.model.set({"value": fixedData.value}, {silent: true});
                        cell.model.set("toDraw", true);
                        //TODO: This solution must be changed, because it's a patch.
                        if (type === "link") {
...
(function () {
    var RadioView = PMDynaform.view.Field.extend({
        ...
        setValue: function (value) {
            var currentValue = this.model.get("value"),
                dataSelected = this.model.findOption(value,"value") || {value: "", label: ""};
            if (value === currentValue && this.firstLoad) {
                this.model.trigger("change:value", this.model, value);
            } else {
                this.model.set("value", value);
...
(function () {
    var TextareaView = PMDynaform.view.Field.extend({
        ...
        /**
         * Call setValue of the TextareaModel
         * @param value
...
(function () {
    ...
    var SuggestView = PMDynaform.view.Field.extend({
        ...
        /**
         * Cal setValue of the Suggest model
         * @param value
         * @returns {SuggestView}
         */
        setValue: function (value) {
...
(function () {
    var DatetimeView = PMDynaform.view.Field.extend({
        ...
        onBlurInput: function () {
            var newVal;
            if (this.datepickerObject.date()) {
                newVal = moment(this.tagControl.val(), this.model.get('format'))
                    .format(this.formatDefault);
                this.tagHiddenToLabel.val(newVal);
            }
        },
        /**
         * events triggered "change" and "validate when the field undergoes a change,
        ...
        checkBinding: function () {
            ...
            if (form && form.onChangeCallback && this.changedValue !== this.getValue()) {
                this.firstLoad = true;
                // fires form onchange Helper functions
                this.changedValue = this.getValue();
                form.onChangeCallback(this.model.get("id"), this.model.get("value"), this.previousValue);
            }
            this.previousValue = this.getValue();
            return this;
        ...
        updateValues: function (event, value) {
            ...
            if (newData) {
                ...
                if (!this.hasValue) {
                    //set value to datetimepicker
                    this.datepickerObject.date(moment(newData.value));
                    if (moment(newData.value).isSame(this.datepickerObject.date())) {
                        newVal = moment(this.tagControl.val(), this.model.get('format'))
                            .format(this.formatDefault);
                        this.tagHiddenToLabel.val(newVal);
                    }
        ...
        /**
         * Validate if the field is required or has an RegeEx
         * @param event
         * @returns {DatetimeView}
         */
        validate: function (event) {
        ...
        render: function () {
            ...
            control.datetimepicker({
                ...
                var domValue = that.tagControl.val(),
                    formatedValue;
                //for setOnchange  purposes
                that.firstLoad = false;
                if (!moment(domValue, that.model.get("format")).isSame(moment(that.oldDate)) && !that.hasValue) {
                    formatedValue = moment(domValue, that.model.get("format"));
                    formatedValue = formatedValue.isValid() ? formatedValue.format(that.formatDefault) : '';
                    that.tagHiddenToLabel.val(formatedValue);
                    that.oldDate =  moment(domValue, that.model.get("format"));
                    that.hasValue = true;
                    that.model.set("value", formatedValue);
                }
        ...
        valueReader: function () {
            ...
            if (value) {
                ...
                // "2017-10-23" or
                // "2017-10-23 04:00:00"
                if (moment(value, this.formatDefault).format(this.formatDefault) !== 'Invalid date') {
                    formatedDate = moment(value, this.formatDefault).format(this.formatDefault);
                } else if (moment(value, this.model.get("format")).format(this.formatDefault) !== 'Invalid date') {
                    formatedDate = moment(value, this.model.get("format")).format(this.formatDefault);
                }
                this.setValue(formatedDate);

            }
            // fill this.previousValue and this.changedValue for setOnchange helper purposes
            if (data) {
                this.previousValue = data.value;
                this.changedValue = data.value;
            }
            return this;
        },
        /**
        ...
        formatData: function (value) {
            ...
            if (value) {
                newData = {
                    value: moment(value, this.formatDefault).format(this.formatDefault),
                    label: moment(value, this.formatDefault).format(originalFormat)
                }
            }
            return newData;
        },
    ...
    var DropdownModel = PMDynaform.model.Field.extend({
        ...
        setFirstOptionInData: function() {
            var options = this.get("options"),
                ...
                index = 0;
            if (options.length) {
                this.set('value', (options[index] && options[index].value) || '');
                this.set("data", options[index] || defaultData);
            }
            if (this.get('view')) {
